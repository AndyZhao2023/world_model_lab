...
class EpisodeBootstrapTest(unittest.TestCase):
    ...
    def test_unsigned_episode_ids_above_int64_max_are_rejected(self):
        too_large = np.asarray(
            [np.iinfo(np.int64).max + 1],
            dtype=np.uint64,
        )

        with self.assertRaisesRegex(ValueError, "int64 range"):
            sample_episode_bootstrap(too_large, seed=0)
        with self.assertRaisesRegex(ValueError, "int64 range"):
            expand_episode_transition_indices(too_large, too_large)
        with self.assertRaisesRegex(ValueError, "int64 range"):
            expand_episode_transition_indices(np.asarray([1]), too_large)
